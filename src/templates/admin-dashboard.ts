...
<body>
  ...
  <main class="main-canvas">
    ...
    <div id="view-ai-agent" class="view-content">
       ...
       <div class="panel-grid" style="margin-top:24px; grid-template-columns: 1fr;">
         <section class="panel">
           <div class="panel-header">
             <h3 class="panel-title">Aderencia Estrategica (90 dias)</h3>
             <div class="flex items-center" style="gap:10px;">
               <select id="strategy-range" class="input-control" style="width:auto; min-width:120px; padding:8px 12px; font-size:0.85rem;">
                 <option value="7" selected>7 dias</option>
                 <option value="14">14 dias</option>
                 <option value="30">30 dias</option>
               </select>
               <button id="btn-export-strategy-alignment" class="btn btn-glass" style="width:auto; padding:8px 14px; font-size:0.8rem;">Exportar CSV</button>
               <button id="btn-refresh-strategy-alignment" class="btn btn-glass" style="width:auto; padding:8px 14px; font-size:0.8rem;">Atualizar</button>
             </div>
           </div>
           <p class="text-sm opacity-60 mt-2 mb-4">Mostra se o Agente Autonomo esta executando conforme o alinhamento: NSM, guardrails e operacoes estagnadas.</p>

           <div class="panel-grid" style="grid-template-columns: 1.2fr 0.8fr; gap:12px; margin-bottom: 12px;">
             <div class="timeline-content">
               <div class="flex items-center justify-between" style="gap:10px; flex-wrap: wrap;">
                 <span class="text-xs opacity-60 uppercase">Acao recomendada agora</span>
                 <span id="strategy-primary-priority" class="badge badge-glass">-</span>
               </div>
               <div id="strategy-primary-title" class="font-bold" style="margin-top:8px;">-</div>
               <p id="strategy-primary-detail" class="text-sm opacity-80" style="margin-top:6px;">-</p>
             </div>
             <div class="timeline-content">
               <div class="flex items-center justify-between" style="gap:10px; flex-wrap: wrap;">
                 <span class="text-xs opacity-60 uppercase">Agente autonomo</span>
                 <span id="autonomous-agent-status" class="badge badge-glass">carregando...</span>
               </div>
               <form method="post" action="/admin/actions/agent-autonomous/toggle" id="autonomous-agent-toggle-form" style="margin-top:10px;">
                 <input type="hidden" id="autonomous-agent-enabled-input" name="enabled" value="false" />
                 <button id="btn-toggle-autonomous-agent" type="submit" class="btn btn-primary" style="width:100%;">...</button>
               </form>
               <p class="text-xs opacity-60" style="margin-top:8px;">Quando desativado, o cron ignora o ciclo automaticamente.</p>
             </div>
           </div>

           <div class="stats-grid" style="margin-bottom: 12px;">
             <div class="stat-card" style="padding:16px; border-radius:16px;">
               <span class="stat-label">NSM Growth (janela)</span>
               <span id="strategy-nsm-growth" class="stat-value" style="font-size:1.1rem;">-</span>
               <span id="strategy-nsm-status" class="badge badge-glass" style="margin-top:8px; width:max-content;">-</span>
             </div>
             <div class="stat-card" style="padding:16px; border-radius:16px;">
               <span class="stat-label">Opt-out Rate (janela)</span>
               <span id="strategy-optout-rate" class="stat-value" style="font-size:1.1rem;">-</span>
               <span id="strategy-optout-status" class="badge badge-glass" style="margin-top:8px; width:max-content;">-</span>
             </div>
             <div class="stat-card" style="padding:16px; border-radius:16px;">
               <span class="stat-label">Operacoes Estagnadas</span>
               <span id="strategy-stale-ops" class="stat-value" style="font-size:1.1rem;">-</span>
               <span id="strategy-stale-status" class="badge badge-glass" style="margin-top:8px; width:max-content;">-</span>
             </div>
             <div class="stat-card" style="padding:16px; border-radius:16px;">
               <span class="stat-label">Reducao Tempo Conversao</span>
               <span id="strategy-ttc-reduction" class="stat-value" style="font-size:1.1rem;">-</span>
               <span id="strategy-ttc-status" class="badge badge-glass" style="margin-top:8px; width:max-content;">-</span>
             </div>
           </div>
           ...
           <div class="table-container" style="margin-top:12px;">
             <table>
               ...
               <tbody id="strategy-decisions-rows">
                 <tr><td colspan="4" class="opacity-40 text-center py-8">Carregando sinais estrategicos...</td></tr>
               </tbody>
             </table>
           </div>
           <div class="text-xs opacity-60" id="strategy-updated-at" style="margin-top:12px;">Aguardando atualizacao...</div>
         </section>
       </div>
  ...
  <script>
    ...
    function renderStrategyStatusBadge(status) {
      const level = String(status || '').toLowerCase();
      if (level === 'ok') {
    ...
    async function loadStrategyAlignment() {
      const refreshBtn = document.getElementById('btn-refresh-strategy-alignment');
      const rangeSelect = document.getElementById('strategy-range');
      const nsmValueEl = document.getElementById('strategy-nsm-growth');
      const nsmStatusEl = document.getElementById('strategy-nsm-status');
      const optOutValueEl = document.getElementById('strategy-optout-rate');
      const optOutStatusEl = document.getElementById('strategy-optout-status');
      const staleValueEl = document.getElementById('strategy-stale-ops');
      const staleStatusEl = document.getElementById('strategy-stale-status');
      const ttcValueEl = document.getElementById('strategy-ttc-reduction');
      const ttcStatusEl = document.getElementById('strategy-ttc-status');
      const primaryTitleEl = document.getElementById('strategy-primary-title');
      const primaryDetailEl = document.getElementById('strategy-primary-detail');
      const primaryPriorityEl = document.getElementById('strategy-primary-priority');
      const agentStatusEl = document.getElementById('autonomous-agent-status');
      const agentToggleBtn = document.getElementById('btn-toggle-autonomous-agent');
      const agentEnabledInput = document.getElementById('autonomous-agent-enabled-input');
      const rowsEl = document.getElementById('strategy-decisions-rows');
      const updatedAtEl = document.getElementById('strategy-updated-at');

      if (!rowsEl) return;
      const days = rangeSelect ? (rangeSelect.value || '7') : '7';
      if (refreshBtn) {
        refreshBtn.disabled = true;
        refreshBtn.textContent = 'Atualizando...';
      }
      ...
      try {
        const response = await fetch('/admin/api/strategy/alignment?days=' + encodeURIComponent(days));
        const data = await response.json();
        if (!response.ok || !data || data.error) {
          throw new Error((data && data.error) || 'Falha ao carregar aderencia estrategica');
        }

        const growthRate = Number(data.metrics?.growthRate || 0);
        const optOutRate = Number(data.metrics?.optOutRate || 0);
        const staleCampaigns = Number(data.metrics?.staleCampaigns || 0);
        const staleJourneys = Number(data.metrics?.staleJourneys || 0);
        const reductionRate = Number(data.metrics?.conversionTimeReductionRate || 0);

        if (nsmValueEl) nsmValueEl.textContent = (growthRate * 100).toFixed(1) + '%';
        if (optOutValueEl) optOutValueEl.textContent = (optOutRate * 100).toFixed(1) + '%';
        if (staleValueEl) staleValueEl.textContent = String(staleCampaigns + staleJourneys);
        if (ttcValueEl) {
          const ttcText = Number.isFinite(reductionRate) ? (reductionRate * 100).toFixed(1) + '%' : '-';
          ttcValueEl.textContent = ttcText;
        }

        if (nsmStatusEl) nsmStatusEl.innerHTML = renderStrategyStatusBadge(data.status?.nsmGrowth);
        if (optOutStatusEl) optOutStatusEl.innerHTML = renderStrategyStatusBadge(data.status?.optOut);
        if (staleStatusEl) staleStatusEl.innerHTML = renderStrategyStatusBadge(data.status?.staleOperations);
        if (ttcStatusEl) ttcStatusEl.innerHTML = renderStrategyStatusBadge(data.status?.conversionTime);

        if (primaryTitleEl) primaryTitleEl.textContent = String(data.primaryAction?.title || '-');
        if (primaryDetailEl) primaryDetailEl.textContent = String(data.primaryAction?.detail || '-');
        if (primaryPriorityEl) {
          const priority = String(data.primaryAction?.priority || 'low');
          const priorityClass = priority === 'high' ? 'badge-warn' : priority === 'medium' ? 'badge-outline' : 'badge-success';
          primaryPriorityEl.className = 'badge ' + priorityClass;
          primaryPriorityEl.textContent = priority;
        }

        const agentEnabled = Boolean(data.agent && data.agent.enabled);
        if (agentStatusEl) {
          agentStatusEl.className = 'badge ' + (agentEnabled ? 'badge-success' : 'badge-warn');
          agentStatusEl.textContent = agentEnabled ? 'ativo' : 'desativado';
        }
        if (agentToggleBtn) {
          agentToggleBtn.textContent = agentEnabled ? 'Desativar Agente' : 'Ativar Agente';
          agentToggleBtn.className = agentEnabled ? 'btn btn-glass' : 'btn btn-primary';
        }
        if (agentEnabledInput) {
          agentEnabledInput.value = agentEnabled ? 'false' : 'true';
        }

        const decisions = Array.isArray(data.recentDecisions) ? data.recentDecisions : [];
        if (!decisions.length) {
          rowsEl.innerHTML = '<tr><td colspan="4" class="opacity-40 text-center py-8">Sem desvios estrategicos recentes.</td></tr>';
        } else {
          rowsEl.innerHTML = decisions.map((item) => {
            ...
            return '<tr>' +
              ...
              '<td><span class="text-xs opacity-60">' + target + '</span></td>' +
              '<td><span class="text-sm opacity-80">' + reason + '</span></td>' +
              '<td><span class="text-xs opacity-60">' + when + '</span></td>' +
            '</tr>';
          }).join('');
        }
        ...
        if (updatedAtEl) {
          const generatedAt = data.generatedAt ? new Date(data.generatedAt).toLocaleString('pt-BR') : '-';
          updatedAtEl.textContent = 'Atualizado em ' + generatedAt + ' • janela: ' + String(data.days || days) + ' dias • metas: NSM +25%, opt-out <5%, estagnacao <= 0, tempo-conversao -30%';
        }
      } catch (error) {
        rowsEl.innerHTML = '<tr><td colspan="4" class="text-center py-8" style="color:#f87171;">Erro ao carregar aderencia estrategica.</td></tr>';
        if (updatedAtEl) updatedAtEl.textContent = 'Falha na atualizacao: ' + String(error);
      } finally {
        if (refreshBtn) {
          refreshBtn.disabled = false;
          refreshBtn.textContent = 'Atualizar';
    ...
    const strategyRefreshBtn = document.getElementById('btn-refresh-strategy-alignment');
    const strategyRangeSelect = document.getElementById('strategy-range');
    const strategyExportBtn = document.getElementById('btn-export-strategy-alignment');
    if (strategyRefreshBtn) strategyRefreshBtn.addEventListener('click', loadStrategyAlignment);
    if (strategyRangeSelect) strategyRangeSelect.addEventListener('change', loadStrategyAlignment);
    if (strategyExportBtn) {
      strategyExportBtn.addEventListener('click', () => {
        const rangeValue = strategyRangeSelect ? strategyRangeSelect.value : '7';
        const url = '/admin/api/strategy/alignment/export.csv?days=' + encodeURIComponent(rangeValue || '7');
        window.open(url, '_blank');
      });
    }
    loadStrategyAlignment();
